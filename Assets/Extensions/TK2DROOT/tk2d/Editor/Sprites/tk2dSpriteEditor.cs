...
class tk2dSpriteEditor : Editor
{
	...
	protected void DrawSpriteEditorGUI()
	{
        ...
        if (targetSprites[0].Collection != null)
        {
            ...
            Color color = EditorGUILayout.ColorField("Color", targetSprites[0].color);
            if (color != targetSprites[0].color)
            {
            	...
            	foreach (tk2dBaseSprite s in targetSprites) 
                {
                    ...
                    if (s is tk2dSprite)
                    {
                        (s as tk2dSprite).Alpha = color.a;
                    }
            	}
            }

            /*if (targetSprites[0] is tk2dSprite)
            {
                tk2dSprite sprite = targetSprites[0] as tk2dSprite;
                Color newColor = EditorGUILayout.ColorField("New Color", sprite.NewColor);
                if (newColor != sprite.NewColor)
                {
                    tk2dUndo.RecordObjects(targetSprites, "Sprite Color");
                    for (int index = 0; index < targetSprites.Length; index++)
                    {
                        sprite = targetSprites[index] as tk2dSprite;
                        if (sprite != null)
                        {
                            sprite.NewColor = newColor;
                        }
                    }
                }
            }*/
